...
csmByte* LAppPal::LoadFileAsBytes(const std::string filePath, csmSizeInt* outSize)
{
    ...
    if (std::filesystem::exists(path)) {
        ...
        if (size == 0) {
            LOGI("Stat succeeded but file size is zero. path:%s", pathStr.c_str());
            return NULL;
        }
    } else {
        LOGI("Stat failed. errno:%d path:%s", errno, pathStr.c_str());
        return NULL;
    }
    ...
    if (!file.is_open()) {
        LOGI("File open failed. path:%s", pathStr.c_str());
        return NULL;
    }
...
void LAppPal::FixMotionJson(std::string& jsonStr)
{
    ...
    if (metaCurveCount == actualCurveCount && metaSegCount == actualSegmentCount &&
    ...
    auto fixInt = [&](const char* key, int newVal) {
        ...
        if (colon == std::string::npos)
            return;
        size_t vs = colon + 1;
        while (vs < jsonStr.size() && (jsonStr[vs] == ' ' || jsonStr[vs] == '\t'))
            vs++;
        size_t ve = vs;
        while (ve < jsonStr.size() && jsonStr[ve] >= '0' && jsonStr[ve] <= '9')
            ve++;
        if (ve > vs)
            jsonStr.replace(vs, ve - vs, std::to_string(newVal));
    };
