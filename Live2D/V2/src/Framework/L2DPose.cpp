...
void PartData::initIndex(ModelContext* context) {
    ...
    for (auto& l : link) l.initIndex(context);
}
...
L2DPose* L2DPose::load(const std::vector<uint8_t>& data) {
    ...
    if (visible == root.end() || !visible->is_array()) return pose;

    for (const auto& poseInfo : *visible) {
        auto groups = poseInfo.find("group");
        if (groups == poseInfo.end() || !groups->is_array()) continue;
        PosePartGroup group;
        for (const auto& partsInfo : *groups) {
            if (!partsInfo.is_object()) continue;
            PartData parts;
