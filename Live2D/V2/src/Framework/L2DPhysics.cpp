...
void L2DPhysics::updateParam(ModelContext* context) {
    ...
    for (auto& ph : mPhysicsList) ph->update(context, t);
}
...
L2DPhysics* L2DPhysics::load(const std::vector<uint8_t>& data) {
    ...
    if (params == root.end() || !params->is_array()) return ret;
    ...
    for (const auto& param : *params) {
        ...
        if (srcList != param.end() && srcList->is_array()) {
            for (const auto& src : *srcList) {
                ...
                PhysicsSrcType t;
                if (ptype == "x") t = SRC_TO_X;
                else if (ptype == "y") t = SRC_TO_Y;
                else if (ptype == "angle") t = SRC_TO_G_ANGLE;
                else continue;  // Python raises here; skip instead
                hair->addSrcParam(t, src.value("id", std::string()),
                                  src.value("scale", 1.0f), src.value("weight", 1.0f));
            }
        }
        ...
        if (tgtList != param.end() && tgtList->is_array()) {
            for (const auto& target : *tgtList) {
                ...
                PhysicsTargetType t;
                if (ptype == "angle") t = TARGET_FROM_ANGLE;
                else if (ptype == "angle_v") t = TARGET_FROM_ANGLE_V;
                else continue;  // Python raises here; skip instead
                hair->addTargetParam(t, target.value("id", std::string()),
                                     target.value("scale", 1.0f), target.value("weight", 1.0f));
            }
        }
