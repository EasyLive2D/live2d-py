...
class GLRenderer(DrawParam):

    def __init__(self, modelContext: ModelContext, version: str):
        super().__init__()
        ...
        self.init(modelContext)

    def init(self, modelContext: ModelContext):
        if self.framebufferObject is None:
            self.createFramebuffer()
        self.clipManager = ClippingManagerOpenGL(self)
        self.clipManager.init(modelContext, modelContext.drawDataList, modelContext.drawContextList)

    def _ensureClipManager(self, modelContext: ModelContext):
        version = modelContext.getInitVersion()
        if self.clipManager is None or version != self._clipInitVersion:
            self.init(modelContext)
            self._clipInitVersion = version

    def preDraw(self, modelContext: ModelContext):
        self._ensureClipManager(modelContext)
        self.setupDraw()
        self.clipManager.setupClip(modelContext)

    def draw(self, modelContext: ModelContext):
        if modelContext.orderList_firstDrawIndex is None:
            print("call Ri_.update() before Ri_.draw() ")
            return

        aP = len(modelContext.orderList_firstDrawIndex)
        self.setupDraw()
        for aK in range(0, aP, 1):
            aN = modelContext.orderList_firstDrawIndex[aK]
            if aN == ModelContext.NOT_USED_ORDER:
                continue

            while True:
                aH = modelContext.drawDataList[aN]
                aI = modelContext.drawContextList[aN]
                if aI.isAvailable():
                    aJ = aI.partsIndex
                    aL = modelContext.partsContextList[aJ]
                    aI.partsOpacity = aL.getPartsOpacity()
                    aH.draw(self, modelContext, aI)

                aO = modelContext.nextList_drawIndex[aN]
    ...
    def bindFramebuffer(self, fb=None):
        if fb is None:
            fb = self.framebufferObject.framebuffer
    ...
    def loadTexture(self, no, width, height, image_data):
        gl = self.gl
